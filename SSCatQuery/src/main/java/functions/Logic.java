...
public class Logic implements HttpFunction {
	private static final Logger logger = Logger.getLogger("SSCatQuery");
	 ...
	 * activities
	 */
	public void SSCatQuery_a1(){
		this.act_exec.put("SSCatQuery_a1", true);
		this.doWork(1.0E-4);
	}
	public void SSCatQuery_work(){
		this.act_exec.put("SSCatQuery_work", true);
		this.doWork(0.0467);
	}
	 ...
	 * Dnodes
	 */
	public void OrNode_SSCatQuery_a1(){
		//OrNode Logic
		List<Pair<String, Double>> distributionData = new ArrayList<>();
		distributionData.add(new Pair<>("SSCatQuery_work", 1.0));
		...
		if(randomChoice=="SSCatQuery_work"){
			SSCatQuery_work();
		}
	}
	public void ReplyNode_SSCatQuery_work(){
		//ReplyNode Logic
		BufferedWriter writer;
		try {
			writer = this.response.getWriter();
			writer.write("SSCatQuery_work[SSCatQuery]");
		} catch (IOException e) {
	...
	public void service(HttpRequest request, HttpResponse response) throws IOException {
		...
		if(this.act_exec.get("SSCatQuery_a1")!=null && this.act_exec.get("SSCatQuery_a1")) {
		  this.act_exec.put("SSCatQuery_a1",false);
		  OrNode_SSCatQuery_a1();
		}
		if(this.act_exec.get("SSCatQuery_work")!=null && this.act_exec.get("SSCatQuery_work")) {
		  this.act_exec.put("SSCatQuery_work",false);
		  ReplyNode_SSCatQuery_work();
		}

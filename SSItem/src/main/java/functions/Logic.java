...
public class Logic implements HttpFunction {
	private static final Logger logger = Logger.getLogger("SSItem");
	 ...
	 * activities
	 */
	public void SSItem_a1(){
		this.act_exec.put("SSItem_a1", true);
		this.doWork(1.0E-4);
	}
	public void SSItem_work(){
		this.act_exec.put("SSItem_work", true);
		this.doWork(0.0234);
	}
	public void SSItem_Query(){
		this.act_exec.put("SSItem_Query", true);
		this.doWork(1.0E-4);
	 ...
	 * Dnodes
	 */
	public void OrNode_SSItem_a1(){
		//OrNode Logic
		List<Pair<String, Double>> distributionData = new ArrayList<>();
		distributionData.add(new Pair<>("SSItem_Query", 1.0));
		...
		if(randomChoice=="SSItem_Query"){
			SSItem_Query();
		}
	}
	public void OrNode_SSItem_Query(){
		//OrNode Logic
		List<Pair<String, Double>> distributionData = new ArrayList<>();
		distributionData.add(new Pair<>("SSItem_work", 1.0));
		EnumeratedDistribution<String> distribution = new EnumeratedDistribution<>(distributionData);
		String randomChoice = distribution.sample();
		if(randomChoice=="SSItem_work"){
			SSItem_work();
		}
	}
	public void ReplyNode_SSItem_work(){
		//ReplyNode Logic
		BufferedWriter writer;
		try {
			writer = this.response.getWriter();
			writer.write("SSItem_work[SSItem]");
		} catch (IOException e) {
	...
	public void service(HttpRequest request, HttpResponse response) throws IOException {
		...
		if(this.act_exec.get("SSItem_a1")!=null && this.act_exec.get("SSItem_a1")) {
		  this.act_exec.put("SSItem_a1",false);
		  OrNode_SSItem_a1();
		}
		if(this.act_exec.get("SSItem_Query")!=null && this.act_exec.get("SSItem_Query")) {
		  this.act_exec.put("SSItem_Query",false);
		  OrNode_SSItem_Query();
		}
		if(this.act_exec.get("SSItem_work")!=null && this.act_exec.get("SSItem_work")) {
		  this.act_exec.put("SSItem_work",false);
		  ReplyNode_SSItem_work();
		}
